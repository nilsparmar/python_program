...
class System_data:
	my_data = {'value': [{'memory_size_MiB': 4096, 'vm': 'vm-1000', 'name': 'SP3-RHEL-68-2x2-REG-1-DATA-1', 'power_state': 'POWERED_ON', 'cpu_count': 4},
			 ...
			 {'memory_size_MiB': 4096, 'vm': 'vm-1022', 'name': 'SP3-RHEL-68-2x2-REG-2-DATA-1', 'power_state': 'POWERED_OFF', 'cpu_count': 4}, 
	...
	def get_vm(self, vm_name):
		...
		for i in range(len(self.my_data['value'])-1):
			if vm_name == self.my_data['value'][i]['name']:
				print(vm_name)
				return True
	...
	def power_on(self, vm_name):
		...
		for i in range(len(self.my_data['value'])-1):
				if vm_name == self.my_data['value'][i]['name']:
					ind = i
					print(self.my_data['value'][ind]['power_state'])
					
					if  self.my_data['value'][ind]['power_state'] == 'POWERED_OFF':
						self.my_data['value'][ind]['power_state']:'POWERED_ON'
						print("Now its turned to POWERED_ON State")
						
					else:
						print("Your Power State is already in 'POWERED_ON' State")
	...
	# 				else:
	# 					print("Your Power State is already in 'POWERED_OFF' State")
